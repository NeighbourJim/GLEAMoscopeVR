...
public class StartAcknowledgeButton : MonoBehaviour, IActivatable, IHideableUI
{
    ...
    {
        _startManager.ShowAcknowledgeCanvas();
    }
    ...
    private void SetAndCheckReferences()
    {
        ...
        Assert.IsNotNull(_canvasGroup, $"<b>[SettingsButton]</b> has no Canvas Group component in parent.");

        _collider = GetComponent<Collider>();
        Assert.IsNotNull(_collider, $"<b>[SettingsButton]</b> has no collider component.");

        _startManager = FindObjectOfType<StartScreenManager>();
        Assert.IsNotNull(_startManager, $"[SettingsButton] {gameObject.name} cannot find StartScreenManager in the scene.");
    }
    #endregion
}
